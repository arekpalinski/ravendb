    ...
    public class ConnectionStringTests : EtlTestBase
    {
        ...
        public void CanAddAndRemoveConnectionStrings()
        {
            using (var store = GetDocumentStore())
            {
                ...
                var elasticConnectionString = new ElasticConnectionString
                {
                    Name = "ElasticConnectionString",
                    Nodes = new[]{"http://127.0.0.1:8080" },
                };

                var result2 = store.Maintenance.Send(new PutConnectionStringOperation<ElasticConnectionString>(elasticConnectionString));
                ...
                Assert.True(record.ElasticConnectionStrings.ContainsKey("ElasticConnectionString"));
                Assert.Equal(elasticConnectionString.Name , record.ElasticConnectionStrings["ElasticConnectionString"].Name);
                Assert.Equal(elasticConnectionString.Nodes, record.ElasticConnectionStrings["ElasticConnectionString"].Nodes);
                ...
                var result5 = store.Maintenance.Send(new RemoveConnectionStringOperation<ElasticConnectionString>(elasticConnectionString));
                ...
                Assert.False(record.ElasticConnectionStrings.ContainsKey("ElasticConnectionString"));
        ...
        public void CanUpdateConnectionStrings()
        {
            using (var store = GetDocumentStore())
            {
                ...
                var result3 = store.Maintenance.Send(new PutConnectionStringOperation<ElasticConnectionString>(elasticConnectionString));
                Assert.NotNull(result3.RaftCommandIndex);
                ...
                var result7 = store.Maintenance.Send(new PutConnectionStringOperation<ElasticConnectionString>(elasticConnectionString));
                ...
                Assert.True(record.ElasticConnectionStrings.ContainsKey("ElasticConnectionString"));
                Assert.Equal("http://127.0.0.1:8081", record.ElasticConnectionStrings["ElasticConnectionString"].Nodes.First());
        ...
        public void CanGetAllConnectionStrings()
        {
            using (var store = GetDocumentStore())
            {
                ...
                var elasticConnectionStrings = new List<ElasticConnectionString>();
                for (var i = 0; i < 5; i++)
                {
                    ...
                    var elasticConnectionStr = new ElasticConnectionString
                    {
                    ...
                    Assert.NotNull(result2.RaftCommandIndex);
                    var result3 = store.Maintenance.Send(new PutConnectionStringOperation<ElasticConnectionString>(elasticConnectionStr));
                    Assert.NotNull(result3.RaftCommandIndex);
                }
                ...
                Assert.NotNull(result.ElasticConnectionStrings);
                ...
                for (var i = 0; i < 5; i++)
                {
                    ...
                    result.ElasticConnectionStrings.TryGetValue($"ElasticConnectionString{i}", out var elastic);
        ...
        public void CanGetConnectionStringByName()
        {
            using (var store = GetDocumentStore())
            {
                ...
                Assert.True(resultSql.ElasticConnectionStrings.Count == 0);
                ...
                Assert.True(resultElastic.ElasticConnectionStrings.Count > 0);
            }
        }
    }
}
