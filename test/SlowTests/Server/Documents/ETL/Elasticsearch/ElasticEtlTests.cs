...
{
    public class ElasticEtlTests : EtlTestBase
    {
        public ElasticEtlTests(ITestOutputHelper output) : base(output)
        {
        }
...
";
        ...
        public void Error_if_script_does_not_contain_any_loadTo_method()
        {
            using (var store = GetDocumentStore())
            {
                ...
                var config = new ElasticEtlConfiguration
                {
                    Name = "test",
                ...
                config.Initialize(new ElasticConnectionString() {Name = "Foo", Nodes = new[] {"http://localhost:9200"}});
...
#pragma warning restore CA1416 // Validate platform compatibility
            ...
            {
                AddEtl(src,
                    new ElasticEtlConfiguration()
                    {
                        ConnectionStringName = "test",
                        Name = "myFirstEtl",
                        ElasticIndexes = {new ElasticIndex {IndexName = "Users", IndexIdProperty = "UserId"},},
                        Transforms =
                        {
                            new Transformation()
                            {
                    ...
                    }, new ElasticConnectionString {Name = "test", Nodes = new[] {"http://localhost:9200"}});
        ...
        {
            var c = new ElasticEtlConfiguration();

            c.Connection = new ElasticConnectionString()
            {
                Name = "Test",
        ...
        public async Task CanTestScript()
        {
            using (var store = GetDocumentStore())
            {
                ...
                var result1 = store.Maintenance.Send(new PutConnectionStringOperation<ElasticConnectionString>(new ElasticConnectionString()
                {
                ...
                {
                    var result = (ElasticEtlTestScriptResult)ElasticEtl.TestScript(
                        new TestElasticEtlScript
                        {
                            DocumentId = "orders/1-A",
                            Configuration = new ElasticEtlConfiguration()
                            {
                                Name = "simulate",
                                ConnectionStringName = "simulate",
                                ElasticIndexes =
                                {
                                    new ElasticIndex {IndexName = "Orders", IndexIdProperty = "Id"},
                                    new ElasticIndex {IndexName = "OrderLines", IndexIdProperty = "OrderId"},
                                    new ElasticIndex {IndexName = "NotUsedInScript", IndexIdProperty = "OrderId"},
                                },
                                Transforms =
                                {
            ...
            AddEtl(store,
                new ElasticEtlConfiguration()
                {
                    ...
                    ElasticIndexes =
                    {
                        new ElasticIndex {IndexName = "Orders", IndexIdProperty = "Id"},
                        new ElasticIndex {IndexName = "OrderLines", IndexIdProperty = "OrderId"},
                        new ElasticIndex {IndexName = "Users", IndexIdProperty = "UserId"},
                    },
                    Transforms =
                    {
                        new Transformation()
                ...
                new ElasticConnectionString {Name = connectionStringName, Nodes = new[] {"http://localhost:9200"}});
        }
