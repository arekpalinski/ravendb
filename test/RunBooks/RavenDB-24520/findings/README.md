...
| **F-9** | medium-low | **open** | A missing old journal in `@SharedJournals/Journals/` fails the WHOLE database load. `IgnoreInvalidJournalErrors` recovers it fully, but the error offers no remedy | [F-9-missing-root-journal-fails-database.md](F-9-missing-root-journal-fails-database.md) |
