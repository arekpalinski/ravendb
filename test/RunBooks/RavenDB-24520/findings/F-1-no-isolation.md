...
**Independent confirmation that the position rule really is dead (Linux, 2026-08-10).** The 16-cell matrix was re-run on a seed whose journal *layout* differs from the Windows one - `Questions_Search last` sits at offset 13,197,312 here versus 11,571,200 there, and the per-cell target offsets differ throughout - yet **every one of the 16 outcomes was identical**. Same corruption kinds, same owners, different positions, same results. If position still governed anything, these two runs would have diverged. Isolation held in all three "owner only" cells: the corrupted transaction's owner came up with `entries=0` while its 5 siblings, the root and the documents were untouched (136,534 docs exact).

The same isolation held for corruption applied **while the server was running** and then hard-killed - see the Scenario 1G section of [../20-LINUX-runbook.md](../20-LINUX-runbook.md). Damage was owner-only there too, so a live writer does not widen the blast radius.
