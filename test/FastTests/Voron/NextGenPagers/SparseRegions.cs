...
public class SparseRegions(ITestOutputHelper output) : StorageTest(output)
{
    ...
    [RavenFact(RavenTestCategory.Voron)]
    public unsafe void StorageReport_ActualSpaceInBytes_ShouldReflectPhysicalDiskSpaceAfterHolePunching()
    {
        Options.ManualFlushing = true;
        var pages = new List<long>();
        using (var wtx = Env.WriteTransaction())
        ...
        if (PlatformDetails.RunningOnMacOsx is false)
        {
            ...
            Assert.True(report.DataFile.ActualSpaceInBytes < report.DataFile.AllocatedSpaceInBytes,
                $"ActualSpaceInBytes ({new Size(report.DataFile.ActualSpaceInBytes, SizeUnit.Bytes)}) should be less than " +
