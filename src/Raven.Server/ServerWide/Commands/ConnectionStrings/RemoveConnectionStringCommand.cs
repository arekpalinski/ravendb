...
namespace Raven.Server.ServerWide.Commands.ConnectionStrings
{
    ...
    public class RemoveElasticConnectionStringCommand : RemoveConnectionStringCommand<ElasticConnectionString>
    {
        protected RemoveElasticConnectionStringCommand()
        {
            // for deserialization
        }

        public RemoveElasticConnectionStringCommand(string connectionStringName, string databaseName, string uniqueRequestId) : base(connectionStringName, databaseName, uniqueRequestId)
        {
        }

        public override void UpdateDatabaseRecord(DatabaseRecord record, long etag)
        {
            record.ElasticConnectionStrings.Remove(ConnectionStringName);
        }
    }
