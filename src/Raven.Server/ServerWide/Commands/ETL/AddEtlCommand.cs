...
namespace Raven.Server.ServerWide.Commands.ETL
{
    ...
    public class AddElasticEtlCommand : AddEtlCommand<ElasticEtlConfiguration, ElasticConnectionString>
    {
        public AddElasticEtlCommand()
        {
            // for deserialization
        }

        public AddElasticEtlCommand(ElasticEtlConfiguration configuration, string databaseName, string uniqueRequestId) : base(configuration, databaseName, uniqueRequestId)
        {
        ...
        public override void UpdateDatabaseRecord(DatabaseRecord record, long etag)
        {
            Add(ref record.ElasticEtls, record, etag);
        }
    }
