...
namespace Raven.Server.Documents.Queries.Dynamic
{
    public class CollectionQueryRunner : AbstractQueryRunner
    {
        ...
        private async Task ExecuteCollectionQueryAsync(QueryResultServerSide<Document> resultToFill, IndexQueryServerSide query, string collection, QueryOperationContext context, bool pulseReadingTransaction, CancellationToken cancellationToken)
        {
                ...
                try
                {
                    using (enumerator)
                    {
                        while (enumerator.MoveNext())
                        {
                            ...
                            await resultToFill.AddResultAsync(document);
