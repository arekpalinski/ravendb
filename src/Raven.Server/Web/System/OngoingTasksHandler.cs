    ...
    public class OngoingTasksHandler : DatabaseRequestHandler
    {
        ...
        public async Task GetConnectionStrings()
        {
            ...
            using (ServerStore.ContextPool.AllocateOperationContext(out TransactionOperationContext context))
            {
                ...
                Dictionary<string, ElasticConnectionString> elasticConnectionStrings;
                ...
                using (var rawRecord = ServerStore.Cluster.ReadRawDatabaseRecord(context, Database.Name))
                {
                    if (connectionStringName != null)
                    {
                        ...
                        (ravenConnectionStrings, sqlConnectionStrings, olapConnectionStrings, elasticConnectionStrings) = GetConnectionString(rawRecord, connectionStringName, connectionStringType);
                    }
                    else
                    {
                        ...
                        elasticConnectionStrings = rawRecord.ElasticConnectionStrings;
                    }
                }
                ...
                await using (var writer = new AsyncBlittableJsonTextWriter(context, ResponseBodyStream()))
                {
                    var result = new GetConnectionStringsResult
                    {
                        ...
                        ElasticConnectionStrings = elasticConnectionStrings
                    };
                    context.Write(writer, result.ToJson());
                }
            }
        ...
        private static (Dictionary<string, RavenConnectionString>, Dictionary<string, SqlConnectionString>, Dictionary<string, OlapConnectionString>, Dictionary<string, ElasticConnectionString>)
            GetConnectionString(RawDatabaseRecord rawRecord, string connectionStringName, ConnectionStringType connectionStringType)
        {
            ...
            var elasticConnectionStrings = new Dictionary<string, ElasticConnectionString>();
            ...
            switch (connectionStringType)
            {
                ...
                case ConnectionStringType.Elastic:
                    var recordElasticConnectionStrings = rawRecord.ElasticConnectionStrings;
                    ...
                    {
                        elasticConnectionStrings.TryAdd(connectionStringName, elasticConnectionString);
                    }
            ...
            return (ravenConnectionStrings, sqlConnectionStrings, olapConnectionStrings, elasticConnectionStrings);
        }
        ...
        private void AssertCanAddOrUpdateEtl(string databaseName, ref BlittableJsonReaderObject etlConfiguration, JsonOperationContext context)
        {
            switch (EtlConfiguration<ConnectionString>.GetEtlType(etlConfiguration))
            {
                ...
                case EtlType.Elastic:
                    ServerStore.LicenseManager.AssertCanAddElasticEtl();
                    break;
                default:
