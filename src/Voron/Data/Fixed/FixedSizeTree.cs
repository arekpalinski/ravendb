...
namespace Voron.Data.Fixed
{
    public unsafe partial class FixedSizeTree : IDisposable
    {
        ...
        private int _largeHeaderAccessorUsages = 0;
        ...
        public byte* DirectAdd(long key, out bool isNew)
        {
            ...
            switch (_type)
            {
                ...
                case RootObjectType.EmbeddedFixedSizeTree:
                    ...
                    break;
                case RootObjectType.FixedSizeTree:
                    using (var headerAccessor = new LargeFixedSizeTreeHeaderAccessor(this))
                    {
                        pos = AddLargeEntry(key, headerAccessor, out isNew);
                    }
                    break;
                default:
        ...
        private byte* AddLargeEntry(long key, LargeFixedSizeTreeHeaderAccessor headerAccessor, out bool isNew)
        {
            FixedSizeTreePage page;

            page = FindPageFor(key, headerAccessor);

            page = ModifyPage(page);

            if (_lastMatch == 0) // update
            {
                headerAccessor.SkipUpdate = true;
                isNew = false;
            ...
            {
                PageSplit(page, key, headerAccessor);

                // now we know we have enough space, or we need to split the parent pageNum
                var addLargeEntry = AddLargeEntry(key, headerAccessor, out isNew);
                isNew = true;
                ValidateTree(headerAccessor);
        ...
        private void ValidateTree(LargeFixedSizeTreeHeaderAccessor headerAccessor)
        {
            if (_type != RootObjectType.FixedSizeTree)
                return;

            var header = headerAccessor.Header;
        ...
        private FixedSizeTreePage FindPageFor(long key, LargeFixedSizeTreeHeaderAccessor headerAccessor)
        {
            var page = GetReadOnlyPage(headerAccessor.Header.RootPageNumber);
            if (_cursor == null)
        ...
        private FixedSizeTreePage PageSplit(FixedSizeTreePage page, long key, LargeFixedSizeTreeHeaderAccessor headerAccessor)
        {
            ...
            if (parentPage == null) // root split
            {
                ...
                headerAccessor.Header.RootPageNumber = parentPage.PageNumber;
                headerAccessor.Header.Depth++;
                headerAccessor.Header.PageCount++;
            ...
            if (page.IsLeaf) // simple case of splitting a leaf pageNum
            {
                ...
                AddSeparatorToParentPage(parentPage, parentPage.LastSearchPosition + 1, separatorKey, newPage.PageNumber, headerAccessor);
            ...
            else // branch page
            {
                ...
                if (page.LastSearchPosition >= page.NumberOfEntries)
                {
                    ...
                    *entry = *page.GetEntry(page.NumberOfEntries - 1);
                    ...
                    AddSeparatorToParentPage(parentPage, parentPage.LastSearchPosition + 1, entry->Key,
                        newPage.PageNumber, headerAccessor);
                ...
                AddSeparatorToParentPage(parentPage, parentPage.LastSearchPosition + 1, newKey, newPage.PageNumber, headerAccessor);
        ...
        private void AddSeparatorToParentPage(FixedSizeTreePage parentPage, int position, long key, long pageNum, LargeFixedSizeTreeHeaderAccessor headerAccessor)
        {
            if ((parentPage.NumberOfEntries + 1) * BranchEntrySize > parentPage.PageMaxSpace)
            {
                parentPage = PageSplit(parentPage, key, headerAccessor);
        ...
        private byte* AddEmbeddedEntry(long key, out bool isNew)
        {
            ...
            using (_tx.Environment.GetTemporaryPage(_tx, out tmp))
            {
                ...
                if (newEntriesCount > _maxEmbeddedEntries)
                {
                    ...
                    using (var headerAccessor = new LargeFixedSizeTreeHeaderAccessor(this))
                    {
                        headerAccessor.Header.NumberOfEntries = newEntriesCount;
                        headerAccessor.Header.ValueSize = _valSize;
                        headerAccessor.Header.Depth = 1;
                        headerAccessor.Header.RootObjectType = RootObjectType.FixedSizeTree;
                        headerAccessor.Header.RootPageNumber = allocatePage.PageNumber;
                        headerAccessor.Header.PageCount = 1;
        ...
        private long DeleteRangeLarge(long start, long end)
        {
            ...
            using (var headerAccessor = new LargeFixedSizeTreeHeaderAccessor(this))
            {
                ...
                while (true)
                {
                    page = FindPageFor(start, headerAccessor);
                    if (page.LastMatch > 0)
                        page.LastSearchPosition++;
                    if (page.LastSearchPosition < page.NumberOfEntries)
                    {
                    ...
                    headerAccessor.Header.NumberOfEntries -= nextPage.NumberOfEntries;

                    var treeDeleted = RemoveEntirePage(nextPage, headerAccessor); // this will rebalance the tree if needed
                ...
                )
                {
                    ...
                    rangeRemoved = RemoveRangeFromPage(page, end, headerAccessor);
        ...
        private int RemoveRangeFromPage(FixedSizeTreePage page, long rangeEnd, LargeFixedSizeTreeHeaderAccessor headerAccessor)
        {
            page = ModifyPage(page);
            ...
            headerAccessor.Header.NumberOfEntries -= (ushort)entriesDeleted;

            if (page.NumberOfEntries == 0)
            {
                RemoveEntirePage(page, headerAccessor);
                return entriesDeleted;
            }
            ...
            if (page.NumberOfEntries == 0)
            {
                if (RemoveEntirePage(page, headerAccessor))
                    return entriesDeleted;
            }
            else
            {
                while (page != null)
                {
                    page = RebalancePage(page, headerAccessor);
                }
            }
            return entriesDeleted;
        }

        private bool RemoveEntirePage(FixedSizeTreePage page, LargeFixedSizeTreeHeaderAccessor headerAccessor)
        {
            ...
            {
                parentPage = RebalancePage(parentPage, headerAccessor);
            }
            return false;
        }
        ...
        private DeletionResult RemoveLargeEntry(long key)
        {
            using (var headerAccessor = new LargeFixedSizeTreeHeaderAccessor(this))
            {
                var page = FindPageFor(key, headerAccessor);
                if (page.LastMatch != 0)
                {
                    headerAccessor.SkipUpdate = true;
                    return new DeletionResult();
        ...
        private FixedSizeTreePage RebalancePage(FixedSizeTreePage page, LargeFixedSizeTreeHeaderAccessor headerAccessor)
        {
            if (_cursor.Count == 0)
            {
                // root page
                ...
                if (headerAccessor.Header.NumberOfEntries <= _maxEmbeddedEntries)
                {
                    System.Diagnostics.Debug.Assert(page.IsLeaf);
                    System.Diagnostics.Debug.Assert(page.NumberOfEntries == headerAccessor.Header.NumberOfEntries);
                    ...
                    headerAccessor.SkipUpdate = true; // because we converted to embedded, large header is no longer valid
        ...
        private class LargeFixedSizeTreeHeaderAccessor : IDisposable
        {
            private readonly FixedSizeTree _fst;
            private readonly bool _readOnly;

            public LargeFixedSizeTreeHeaderAccessor(FixedSizeTree fst, bool readOnly = false)
            {
                if (readOnly == false && fst._largeHeaderAccessorUsages++ > 0)
                    ThrowAccessorAlreadyInUse(fst._treeName);

                _fst = fst;
                _readOnly = readOnly;

                Header = *(FixedSizeTreeHeader.Large*)_fst._parent.DirectRead(_fst._treeName);
            }

            public FixedSizeTreeHeader.Large Header;

            public bool SkipUpdate;

            private static void ThrowAccessorAlreadyInUse(Slice treeName)
            {
                throw new InvalidOperationException($"Large header accessor of {treeName} fixed size tree is already in use");
            }
            ...
            public void Dispose()
            {
                ...
                System.Diagnostics.Debug.Assert(_fst._tx.Flags == TransactionFlags.ReadWrite);

                _fst._largeHeaderAccessorUsages--;

                if (SkipUpdate)
                    return;

                System.Diagnostics.Debug.Assert(_fst._type == RootObjectType.FixedSizeTree);

                using (var header = _fst._parent.DirectAdd(_fst._treeName, sizeof(FixedSizeTreeHeader.Large)))
                {
                    var largeHeaderPtr = (FixedSizeTreeHeader.Large*)header.Ptr;
                    *largeHeaderPtr = Header;
                }
            }
        }
    }
